...
class RocketMenu(Select):
    def __init__(self, rockets: list[Rocket]):
        ...
        super().__init__(placeholder = "View available rockets!", min_values = 1, max_values = len(rockets), options = options)
    ...
    async def callback(self, interaction):
        business_data = await interaction.client.db.business.find_one({"owner_id": interaction.author.id})
        ...
        for value in interaction.values:
            rocket = Rocket.from_dict(interaction.client.get_cog("Business").rockets[value])
            total += rocket.price
        ...
        await interaction.client.db.business.update_one({business_data, {"$inc": {"money": -total}}})
        await interaction.response.send_message(f"You bought {', '.join([rocket.name for rocket in rockets])} for {total}!")
...
class SellRocketMenu(RocketMenu):
    async def callback(self, interaction):
        ...
        await interaction.client.db.business.update_one({business_data, {"$inc": {"money": total}}})
        await interaction.response.send_message(f"You sold {', '.join([rocket.name for rocket in rockets])} for {total}!")
...
class Business(commands.Cog):
    ...
    async def business(self, ctx: commands.Context):
        ...
        embed.add_field(name = "create", value = "Create a new business if you don't already have one.", inline=False)
        ...
        embed.add_field(name = "transfer", value = "Transfer ownership of your business.", inline=False)
    ...
    @business.command(name = "take_off", aliases = ['tf'], description = "Take off your rockets, gaining you money.")
    async def business_take_off(self, ctx):
        ...
        income = 0
        for rocket in rockets:
            seconds = int(time()) - business_data["last_claim_time"] # CurrentTime - LastClaimTime = Difference
            multiplier = seconds * rocket["income_per_second"] # Difference * IncomePerSecond
            income += rocket.rate * multiplier # HowMuchMoneyRocketMakes * Difference
        await self.bot.db.business.update_one(business_data, {"$inc": {"money": income}})
        await self.bot.db.business.update_one(business_data, {"$set": {"last_claim_time": int(time())}})
    ...
    @commands.group(name = "rocket", invoke_without_command = True, description = "The top level rocket command. Use a subcommand.", case_insensitive = True)
    async def rocket(self, ctx):
        embed = Embed(title = "Rocket Command", description = "Use a subcommand to get information about rockets.", color = 0x00ff00)
        
        embed.set_author(name = "Rocket Command", icon_url = self.bot.user.avatar_url)
        ...
        embed.add_field(name = "info", value = "Get information about a rocket.", inline = False)

        await ctx.reply(embed = embed)
    
    @rocket.command(name = "buy", description = "Buy a rocket.", aliases = ['b'])
    async def rocket_buy(self, ctx):
        await ctx.reply("Select a rocket to buy from the menu below.", view = View().add_item(RocketMenu()))


    @rocket.command(name = "sell", description = "Sell a rocket.", aliases = ['s'])
    async def rocket_sell(self, ctx):
        await ctx.reply("Select a rocket to sell from the menu below.", view = View().add_item(SellRocketMenu()))
    ...
    async def rocket_list(self, ctx):
        ...
        embed = Embed(title = "Rocket List", description = "All of your rockets.", color = 0x00ff00)
        embed.set_author(name = "Rocket List", icon_url = self.bot.user.avatar_url)
        embed.set_footer(text = f"Requested by {ctx.author}", icon_url = ctx.author.avatar_url)

        for rocket in business["rockets"]:
            embed.add_field(name = rocket["name"], value = f"Cost: {rocket['cost']}", inline = False)
